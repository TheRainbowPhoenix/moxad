...
<link href="./main_style.css" rel=stylesheet type="text/css">
	...
	<% net_Web_show_value("SRV_IGMPSNOOPV3_SET"); %>
	...
	function tblAddForamt(idx, newdata, igmp_enable){
		...
		// Index
		newdata[0] = index;
        if(check_vid_bridge(SRV_VLAN[idx].vlanid)){
            index++;
        }
	...
	function tblInit(){
		...
		// Create table
		for(var i = 0; i < SRV_VLAN.length; i++) {
        /*justinjz_huang modify for brige_mode , show vid not in birdge*/
//            if(check_vid_bridge(SRV_VLAN[i].vlanid)){
                ...
                if(check_vid_bridge(SRV_VLAN[i].vlanid)){
        			...
        			tableaddRowHidden_2("tbl_igmpsnoopv3", 0, newdata, "center");
                }
  ...
  //         }
	...
	function Activate(form)
	{
		...
		if (document.getElementById('txt_igmp_query_interval').value  < 12 || document.getElementById('txt_igmp_query_interval').value  > 600){
			alert("The range of Quierier interval value should be between 12 and 600 !!!");
			return;
		}
		for(var idx = 0; idx < SRV_VLAN.length; idx++) {
			// Vid
