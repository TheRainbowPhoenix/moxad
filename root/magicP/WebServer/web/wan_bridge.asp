...
var wtype = {
	...
	dns1:5, dns2:5, dns3:5, bcast:3, bcastIP:3, proxy_arp:3//, htnm:4, domn:4, wmac:7
};
...
function check_vlan_inuse( vlanid){
    ...
    for(i=0;i<SRV_VPLAN.length;i++){
        if(SRV_VPLAN[i].bridge_group_id !=0){
            if(SRV_VPLAN[i].pvid == vlanid){
                rv =0;
            }
        }
		else{
			/* if this vid is occupied by zone-base bridge. */
			if(check_brg_occpupy_vid(vlanid)){
				rv = 0;
			}
		}
    }
...
/*
	判斷是否為合法的符號(a~z, A~Z, 0~9, '.', '-', '_', '@')
	合法回傳:	0
	不合法回傳:	1
*/
function isSymbol_forPPPoE(obj, ObjName){
	var TempObj;
	TempObj=obj.value;
...
function Send(form) {
	...
	if(myForm.wtyp.value==4){
		if(isNull(myForm.eusr.value) ||	isNull(myForm.epwd.value)){
			...
			if( isSymbol_forPPPoE(myForm.eusr, PPPoE_Dialup + 'User Name')
	...
	if(myForm.vmac.value!=""){
		if(!MacAddrIsOK(myForm.vmac, VIRTUAL_MAC_)){
			return;
		}
	}else{
		myForm.vmac.value="00:00:00:00:00:00"
	}
...
<input type="hidden" id=proxy_arp name="proxy_arp">
<% net_Web_csrf_Token(); %>
...
<table cellpadding=1 cellspacing=2 width=600px>
 <tr class=r0>  
  <td style="width:100px"><script language="JavaScript">doc(VIRTUAL_MAC_)</script></td></tr>
 <tr>
