...
if (!debug) {
	...
	{{ net_Web_show_value('SRV_DHCP_PIP') | safe }}
	var Route_Policy_MAX = {{ net_Web_Get_SYS_PORTS() | safe }};
...
{	
    ...
    if(SRV_VPLAN[port-1].bridge_group_id != 0){
        alert("this port is bridge port, not support Port-based IP Assignment")
        return;
    }
	for(i = 0 ; i < SRV_DHCP_PIP.length ; i++)
	{	
