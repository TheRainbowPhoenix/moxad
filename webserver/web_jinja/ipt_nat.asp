...
if (!debug) {
	...
	var WanInfo = [ {{ net_Web_IPT_WANInfo_WriteValue() | safe }} ];
	if(ModelVLAN == RETURN_TRUE && (ProjectModel == MODEL_EDR_G903 || ProjectModel == MODEL_EDR_G902)){
		var vlanInfo = [ {{ net_Web_NAT_vconf_info_WriteValue() | safe }} ];
	}
	...
	var one_to_one_ifs_list = [ {{ net_Web_nat_show_ifs_table() | safe }}  ];
...
var ifs1 = [ {{ net_Web_NAT_IFS_WriteValue(0) | safe }} ];
var ifs2 = [ {{ net_Web_NAT_IFS_WriteValue(1) | safe }} ];
var brg_ifs	= [ {{ net_Web_NAT_show_brgIfs_WriteValue() | safe }} ];

var vrrp_binding_tbl = [ {{ net_Web_NAT_vrrpBinding_WriteValue() | safe }} ];
