...
if (!debug) {
	...
	//{{ net_Web_show_value("SRV_LAN") | safe }}
...
function VConf_Addformat(mod,i)
{
	...
	for(k in SRV_VCONF_type){
		...
		if(k=="vmac"){
			if(mod==0){
				if(if_data[table_index.value][i][k]=="00:00:00:00:00:00"){
					newdata0[j]="--";
				}
			}else{
				if(document.getElementById('LanForm')[k].value=="00:00:00:00:00:00" ||
					document.getElementById('LanForm')[k].value==""){
					newdata0[j]="--";
				}
			}
	...
	for(i=0; i < SRV_VLAN.length; i++){
		...
		if(check_vlan_inuse(SRV_VLAN[i].vlanid, 0)){
            ...
            for(j =0; j < SRV_VPLAN.length; j++){
                if(SRV_VLAN[i].vlanid == SRV_VPLAN[j].pvid){
                    if(SRV_VPLAN[j].bridge_group_id == 0){
                        if(check_vid(SRV_VLAN[i].vlanid) && !check_brg_occpupy_vid(SRV_VLAN[i].vlanid)){
           	    		  	flag=1;
                        }else{
							flag=0;
						}
		...
		if(flag){
			var varItem = new Option(SRV_VLAN[i].vlanid, SRV_VLAN[i].vlanid);
            document.getElementById("vid").options.add(varItem);
            used_vid[idx] = SRV_VLAN[i].vlanid;
			idx++;
		}
	}
...
/******************************************************************
 *	Purpose:	check the vid is occupied by zone-base bridge or not.
 * 	Inputs: 	vid - vlan id
 * 	Return:		0: false
 *				1: true
 * 	Author: 	Kevin Haung
 * 	Date: 		2015/02/16
 ******************************************************************/
function check_brg_occpupy_vid(vid)
{
	var i;
	var member_vid_idx;
	var member_vid_var;
	...
	for(i=0; i < SRV_ZONE_BRG.length; i++){
		for(member_vid_idx = 0; member_vid_idx < SRV_VPLAN.length; member_vid_idx++){

			member_vid_var = "member_vid" + member_vid_idx;

			if(vid == SRV_ZONE_BRG[i][member_vid_var]){
				return 1;
			}
		}
	}
...
function VConf_Tabbtn_sel(form, sel)
{
	if(sel == 0 || sel == 2){
	    ...
	    if(!IpAddrNotMcastIsOK(form.v_ip, IP_Address) || !NetMaskIsOK(form.v_mask, Subnet_Mask))
		    return;

		if(!(IpAddrMaskIsOK(form.v_ip, form.v_mask,IP_Address)) || !(NetMaskIsOK(form.v_mask, Subnet_Mask)))
		{
			return;
		}
		if(form.vmac.value!=""){
			if(!MacAddrIsOK(form.vmac, VIRTUAL_MAC_)){
				return;
			}
		}else{
			form.vmac.value="00:00:00:00:00:00"
		}
		...
		if(hasWhiteSpace(form.ifname.value) != 0){ // whitespace check
	...
	for(var i=2;i<document.getElementById("show_available_table").getElementsByTagName("tr").length;i++){
		var tmp = document.getElementById("show_available_table").getElementsByTagName("tr")[i];
		tmp.getElementsByTagName("td")[5].style.textAlign = "center";
	}
}
...
<table cellpadding=1 cellspacing=1 border=0 width=100%>
 ...
 <tr align="left">
  ...
  <td><script language="JavaScript">doc(VIRTUAL_MAC_)</script></td>
  <td><input type="text" id=vmac name="vmac" size="17" maxlength="17"></td>
  <td><input type="hidden" id=v_routing name="routing"></td></tr>
  <td><input type="hidden" id=v_proxy_arp name="proxy_arp"></td></tr>
...
<tr class=r0>
 ...
 <tr align="left">
  ...
  <th width=120px class="s0"><script language="JavaScript">doc(VIRTUAL_MAC_)</script></th>
