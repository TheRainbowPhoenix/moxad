...
if (!debug) {
	...
	{{ net_Web_show_value('SRV_VRRP') | safe }}	
}
...
if (!debug) {
	...
	var CurrentStatus = [ {{ net_webVrrpCurrentStatus() | safe }} ];
	var CurrentIp = [ {{ net_webVrrpCurrentIp() | safe }} ];
}

var vrrp_ifs = [ {{ net_Web_Write_vrrpIFS_IntegerValue() | safe }} ];

if(ProjectModel == MODEL_EDR_G903){
	var vrrp_track_ifs = [ { ifs:'0', vid:'0', text:'--' }, { ifs:'255', vid:'255', text:'WAN1+WAN2' }, {{ net_Web_Write_vrrpIFS_IntegerValue() | safe }} ];
}
else{
	var vrrp_track_ifs = [ { value:'0', text:'--' }, {{ net_Web_Write_vrrpIFS_IntegerValue() | safe }} ];
}
